...
public class HaiDao : MonoBehaviour
{
    ...
    void SL()
    {
        txtSL.text = ManagerData.instance.dataTool.data[2].SoLuong.ToString();
    }
    ...
    void ChatCay()
    {
        if (show == 0)
        {
            if (ManagerData.instance.dataTool.data[2].SoLuong > 0)
            {
                GetComponent<SpriteRenderer>().sprite = thatSprite;
                time = 300;
                OpenDialog.instance.Close();
                InventoryManager.number += 1;
                ManagerData.instance.dataTool.data[2].SoLuong -= 1;
